...
export class Delta {
   ...
   */
    static createDelta(detection: HandOverDetection,
        modifiedValues: ModifiedValues): Delta {
        return new Delta({
            direction: AxisAngle.getRotationBetween(modifiedValues.currentDirection,
   ...
   */
    applyDirectionDelta(direction: vec3): vec3 {
        ...
        return AxisAngle.applyRotation(this.config.direction.multipliedBy(this.config.multiplier),
            direction);
    }
...
export default class RgbIrTransitionJumpSuppressor {
     ...
     */
        ...
        for (const pair of jointPairs) {
            ...
            this.bones.push({
                ...
                smoothedLength: new TimedScalarContainer(WindowMode.FRAME,
    ...
    private detectHandOver(): HandOverDetection {
        ...
        for (const bone of this.bones) {
            ...
            if (previousLength !== null) {
                ...
                this.log.v(`${bone.begin.getAttachmentPoint().name} - ${
      ...
      RgbIrTransitionJumpSuppressorConfigDefault.boneRatioChangeLimit
            ...
            this.log.d(`Model swich detected: ${sign === 1 ? 'HLMT' : 'UL'}, diff: ${(
                ratioDifference * 100
    ...
    private computeModifiedValues(currentDirection: vec3,
        previousDirection: vec3,
        currentLocus: vec3,
   ...
   */
    update(currentDirection: vec3, currentLocus: vec3): void {
      ...
      this.previousLocus !== undefined
            ...
            const delta = Delta.createDelta(detection,
                this.computeModifiedValues(currentDirection,
            ...
            for (const delta of this.deltas) {
                this.log.d(`decay: ${delta.canDecay}, id: ${delta.id}, to model: ${
