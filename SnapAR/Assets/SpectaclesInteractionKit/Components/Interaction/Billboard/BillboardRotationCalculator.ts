...
export default class BillboardRotationCalculator {
    ...
    private calculateRotationQuaternion(rotationRadians: number,
        axisVector: vec3): quat {
   ...
   */
    private calculateAxisAngle(axisVector: vec3,
        forwardVector: vec3,
        cameraVector: vec3,
        originVector: vec3): number {
    ...
    public getRotation(axisVector: vec3,
        forwardVector: vec3,
        cameraVector: vec3,
        originVector: vec3): quat {
        if (this.skipRotation()) {
            return quat.quatIdentity();
        }
        const angle = this.calculateAxisAngle(axisVector,
            forwardVector,
            cameraVector,
            originVector);
        const rotationRadians = this.calculateAxisRotation(angle);

        const rotationQuaternion = this.calculateRotationQuaternion(rotationRadians,
            axisVector);
   ...
   */
    public resetRotation(axisVector: vec3,
        forwardVector: vec3,
        cameraVector: vec3,
        originVector: vec3) {
        const angle = this.calculateAxisAngle(axisVector,
