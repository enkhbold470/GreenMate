...
export class SeededRandomNumberGenerator {
   ...
   */
    public getRandomNumberArrayInRangeNoDuplicates(rangeMin: number,
        rangeMax: number,
        numRandomNumbers: number): number[] {
        if (rangeMin >= rangeMax) {
            throw new Error(`Illegal arguments, rangeMin (${rangeMin}) cannot be >= rangeMax (${rangeMax})`);
        }

        if (numRandomNumbers > rangeMax - rangeMin) {
            throw new Error(`Illegal arguments, numRandomNumbers (${numRandomNumbers}) cannot be > rangeMax - rangeMin (${
                rangeMax - rangeMin
            })`);
        }
   ...
   */
    randomPointInAABB(minPoint: vec3, maxPoint: vec3): vec3 {
      ...
      minPoint.z > maxPoint.z
        ...
        const x = MathUtils.remap(this.randomRange(0, 1)(),
            1,
            0,
            maxPoint.x,
            minPoint.x);
        const y = MathUtils.remap(this.randomRange(0, 1)(),
            1,
            0,
            maxPoint.y,
            minPoint.y);
        const z = MathUtils.remap(this.randomRange(0, 1)(),
            1,
            0,
            maxPoint.z,
            minPoint.z);
