...
implements PinchDetectionStrategy {
    ...
    private update() {
        const adjustedThumbPosition = this.adjustThumbDepthToPointer(this.config.thumbTip.position,
    ...
    private updatePinchProximity(): void {
        ...
        const mappedDistance = mapValue(clamp(distance, MIN_PINCH_DISTANCE, MAX_PINCH_DISTANCE),
            MAX_PINCH_DISTANCE,
            MIN_PINCH_DISTANCE,
            1,
            0);
