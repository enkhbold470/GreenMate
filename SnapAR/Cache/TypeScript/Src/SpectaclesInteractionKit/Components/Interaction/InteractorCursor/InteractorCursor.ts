...
export class InteractorCursor extends BaseScriptComponent {
  ...
  onAwake() {
      ...
      this.viewModel = new CursorViewModel(this.enableCursorSnapping,
          this.enableFilter,
  ...
  private updateWorldCursor(data: CursorData) {
      ...
      if (data.interactionStrength !== null) {
          this.circleVisual.circleSquishScale = MathUtils.lerp(DEFAULT_IDLE_SCALE,
  ...
  private updateManipulateLine(data: ManipulateLineData) {
      this.manipulateLine.points = this.getCurvedLinePoints(data.origin,
          data.endPoint,
          data.delta);
  }
   ...
   */
  private getCurvedLinePoints(start: vec3,
      end: vec3,
      curveOffset: vec3 | null): vec3[] {
