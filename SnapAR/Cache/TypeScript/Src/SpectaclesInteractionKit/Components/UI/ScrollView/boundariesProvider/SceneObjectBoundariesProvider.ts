...
export abstract class SceneObjectBoundariesProvider extends BoundariesProvider {
    ...
    protected startingPosition = this.localPointToParentPoint(this.screenTransform,
        vec2.zero());
   ...
   */
    ...
    get boundaries(): Rect {
        const offsetPosition = this.localPointToParentPoint(this.screenTransform,
            vec2.zero()).sub(this.startingPosition);
        return Rect.create(this.startingBoundaries.left + offsetPosition.x,
   ...
   */
    recomputeStartingBoundaries(): void {
        this.startingPosition = this.localPointToParentPoint(this.screenTransform,
            vec2.zero());
        this.startingBoundaries = this.getBoundaries();
    }
  ...
  protected createScreenTransformRectBoundaries(screenTransform: ScreenTransform): Rect {
      const topLeftCorner = this.localPointToParentPoint(screenTransform,
          new vec2(-1, 1));

      const bottomRightCorner = this.localPointToParentPoint(screenTransform,
          new vec2(1, -1));

      return Rect.create(topLeftCorner.x,
          bottomRightCorner.x,
          bottomRightCorner.y,
          topLeftCorner.y);
  }

  private localPointToParentPoint(screenTransform: ScreenTransform,
      position: vec2) {
