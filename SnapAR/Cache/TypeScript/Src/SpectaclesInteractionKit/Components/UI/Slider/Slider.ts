...
export class Slider extends BaseScriptComponent {
  ...
  @input
  @hint('The SceneObject representing the knob of the slider which will be moved along the path between the positions provided by sliderMin and sliderMax when the value is updated. Please ensure the SceneObject has an Interactable component attached.')
  ...
  onAwake() {
      ...
      if (this.editEventCallbacks) {
          if (this.customFunctionForOnHoverEnter) {
              this.onHoverEnter.add(createCallback<void>(this.customFunctionForOnHoverEnter,
                  this.onHoverEnterFunctionNames));
          }

          if (this.customFunctionForOnHoverExit) {
              this.onHoverExit.add(createCallback<void>(this.customFunctionForOnHoverExit,
                  this.onHoverExitFunctionNames));
          }

          if (this.customFunctionForOnSlideStart) {
              this.onSlideStart.add(createCallback<number>(this.customFunctionForOnSlideStart,
                  this.onSlideStartFunctionNames));
          }

          if (this.customFunctionForOnSlideEnd) {
              this.onSlideEnd.add(createCallback<number>(this.customFunctionForOnSlideEnd,
                  this.onSlideEndFunctionNames));
          }

          if (this.customFunctionForOnValueUpdate) {
              this.onValueUpdate.add(createCallback<number>(this.customFunctionForOnValueUpdate,
                  this.onValueUpdateFunctionNames));
          }

          if (this.customFunctionForOnMinValueReached) {
              this.onMinValueReached.add(createCallback<number>(this.customFunctionForOnMinValueReached,
                  this.onMinValueReachedFunctionNames));
          }

          if (this.customFunctionForOnMaxValueReached) {
              this.onMaxValueReached.add(createCallback<number>(this.customFunctionForOnMaxValueReached,
  ...
  private setupInteractable() {
      ...
      this.unsubscribeBag.push(this.interactable.onDragStart.add((event: DragInteractorEvent) => {
      ...
      this.unsubscribeBag.push(this.interactable.onDragUpdate.add((event: DragInteractorEvent) => {
          this.updateSliderState({
              dragVector:
            event.interactor.dragType !== DragType.Touchpad
                ? event.planecastDragVector
  ...
  private calculateDisplayValueFromRawValue(rawValue: number): number {
      const displayValue = MathUtils.remap(rawValue,
          this.trackState.trackMin,
          this.trackState.trackMax,
          this._minValue,
          this._maxValue);
  ...
  private calculateRawValueFromDisplayValue(displayValue: number): number {
      return MathUtils.remap(displayValue,
          this._minValue,
          this._maxValue,
          this.trackState.trackMin,
          this.trackState.trackMax);
  ...
  private updateUI(): void {
      this.sliderKnobScreenTransform.anchors.setCenter(vec2.lerp(this.minBound,
          this.maxBound,
