...
export class ScrollBar extends BaseScriptComponent {
  ...
  @input
  @hint('The mesh visual of the scroll bar, used to calculate the height offset that should be used to prevent the mesh from extending past the canvas. This mesh will also be disabled whenever setting this component to disabled.')
  @allowUndefined
    private _scrollBarMeshVisual: RenderMeshVisual | null = null;

  @input
  @hint('How far (in cm) the top edge of the ScrollBar mesh should sit from the edge of the canvas when at the top of the content.')
  ...
  private onScroll(event: ScrollViewEventArgs) {
      ...
      position.y =
      this.yOrigin +
      MathUtils.lerp(this.boundingHeight,
          -this.boundingHeight,
