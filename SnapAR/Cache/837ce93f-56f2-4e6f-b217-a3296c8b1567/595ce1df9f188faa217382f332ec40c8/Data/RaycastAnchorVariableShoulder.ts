...
export default class RaycastAnchorVariableShoulder extends RaycastBase {
   ...
   */
    private getVerticalHeadTransform(): mat4 {
        ...
        return mat4.compose(this.camera.getWorldPosition(),
            levelRotation,
    ...
    private estimateShoulderPosition(handPosition: vec3): vec3 {
        ...
        const elevationIntervalElbow = MathUtils.clamp(inverseLerp(ELBOW_LOCATION.y + ELBOW_TRACKED_RADIUS,
            ELBOW_LOCATION.y - ELBOW_TRACKED_RADIUS,
            handElevationHeadset),
        0,
        1);

        const lateralIntervalElbow = MathUtils.clamp(inverseLerp(ELBOW_LOCATION.x + ELBOW_TRACKED_RADIUS_HORIZONTAL,
        ...
        const variableElbowOffset = ELBOW_LOCATION.add(new vec3(Math.cos(lerpInvX * Math.PI * -1) * ELBOW_RADIUS_HORIZONTAL,
    ...
    getRay(): RaycastInfo | null {
        ...
        const shoulder = this.shoulderOneEuroFilter.filter(estimatedShoulder,
            getTime());
        ...
        const smoothTargetingRay = this.directionOneEuroFilter.filter(targetingRay,
            getTime());
