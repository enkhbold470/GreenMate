...
let CapsuleMeshCustomizer = class CapsuleMeshCustomizer extends BaseScriptComponent {
    ...
    buildCircle(originX, radius, isEnd) {
        ...
        for (let i = -Math.PI / 2; i < 1.5 * Math.PI; i = i + Math.PI / this.radianStepCount) {
    ...
    buildEndCap(originX, radius, isRight) {
        ...
        for (let i = isRight ? step : -radius + step; i < (isRight ? radius : 0); i = i + step) {
    ...
    linkCircleIndices(circleIndexA, circleIndexB) {
        ...
        for (let i = firstIndex; i < (circleIndexA + 1) * numPoints - 1; i = i + 1) {
