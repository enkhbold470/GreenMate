...
export class InteractableManipulation extends BaseScriptComponent {
  ...
  @input
  @hint('Root SceneObject of the set of SceneObjects to manipulate. If left blank, this script\'s SceneObject will be treated as the root. The root\'s transform will be modified by this script.')
  @allowUndefined
    private manipulateRootSceneObject: SceneObject | null = null;
  @input
  @widget(new SliderWidget(0, 1, 0.05))
  @hint('The smallest this object can scale down to, relative to its original scale. A value of 0.5 means it cannot scale smaller than 50% of its current size')
  /**
  ...
  @hint('The largest this object can scale up to, relative to its original scale. A value of 2 means it cannot scale larger than twice its current size')
  /**
  ...
  @hint('Enable rotation about all axes or a single world axis (x,y,z) when using to two hands.')
  ...
  private updateStartValues(): void {
      ...
      if (interactors.length === 1) {
        ...
        interactors[1].activeTargetingMode === TargetingMode.Direct;
          ...
          const dualInteractorDirection = this.getDualInteractorDirection(interactors[0],
              interactors[1]);
  ...
  private cacheTransform() {
      ...
      this.originalLocalTransform = {
          transform: mat4.compose(this.transform.getLocalPosition(),
  ...
  private getTriggeringInteractors(): Interactor[] {
      ...
      this.interactionManager.getInteractorsByType(this.interactable.triggeringInteractor);

      if (interactors === null) {
          this.log.w(`Failed to retrieve interactors on ${this.getSceneObject().name}: ${
  ...
  private invokeEvents(translateEvent: Event<TranslateEventArg> | null,
      rotationEvent: Event<RotationEventArg> | null,
      scaleEvent: Event<ScaleEventArg> | null,
      manipulationEvent: Event<TransformEventArg> | null): void {
  ...
  private getDualInteractorDirection(interactor1: Interactor,
      interactor2: Interactor): quat | null {
      if (
          interactor1 === null ||
  ...
  private singleInteractorTransform(interactor: Interactor): void {
      ...
      if (this.enableTranslation) {
          ...
          if (this.cachedTargetingMode === TargetingMode.Direct) {
              ...
              this.smoothedStretch = MathUtils.lerp(this.smoothedStretch,
  ...
  private dualInteractorsTransform(interactors: Interactor[]): void {
      ...
      const dualDirection = this.getDualInteractorDirection(interactors[0],
          interactors[1]);
      ...
      if (this.enableTranslation) {
          ...
          0.5;
              const zDistance = Math.min(dualRaycastDistance,
  ...
  private calculateStretchFactor(interactor: Interactor): number {
      ...
      let finalStretchAmount = MathUtils.clamp(dragAmount * distanceFactor,
          minStretch,
          maxStretch);
      ...
      if (interactor.inputType === InteractorInputType.Mobile) {
        ...
        mobileDragVector.z === 0
          ...
          this.mobileStretch = Math.min(maxStretch - finalStretchAmount,
  ...
  private clampUniformScale(scale: vec3, minScale: vec3, maxScale: vec3): vec3 {
     ...
     */
      const minRatio = Math.min(scale.x / minScale.x,
          scale.y / minScale.y,
          scale.z / minScale.z);
      const maxRatio = Math.min(scale.x / maxScale.x,
          scale.y / maxScale.y,
          scale.z / maxScale.z);
