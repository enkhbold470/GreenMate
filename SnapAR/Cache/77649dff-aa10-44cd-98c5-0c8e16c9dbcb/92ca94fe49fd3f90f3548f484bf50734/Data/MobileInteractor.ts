...
export class MobileInteractor extends BaseInteractor {
  ...
  @input
  @hint('Initialize Mobile Input Data Provider with Position and Rotation Filtering')
  ...
  protected updateDragVector(): void {
      if ((this.currentTrigger & InteractorTriggerType.Select) !== 0) {
          const touchpadDragVector = this.touchpadDragProvider.getDragVector(this.getTouchpadDragPoint(),
              this.currentInteractable?.enableInstantDrag ?? null);
          const sixDofDragVector = this.sixDofDragProvider.getDragVector(this.getSixDofDragPoint(),
              this.currentInteractable?.enableInstantDrag ?? null);

          this.currentDragVector = this.dragProvider.currentDragVector;

          this.planecastDragProvider.getDragVector(this.planecastPoint,
              this.currentInteractable?.enableInstantDrag ?? null);
      } else {
  ...
  private getTouchpadDragPoint(): vec3 | null {
      if (this.touchpadCurrentPosition !== null) {
          return new vec3(
              ...
              0).uniformScale(this.touchpadScrollSpeed);
      }
      return null;
  }
