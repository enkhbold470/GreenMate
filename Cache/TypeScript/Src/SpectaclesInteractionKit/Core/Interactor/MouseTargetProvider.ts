...
export default class MouseTargetProvider extends TargetProvider {
    ...
    constructor(private interactor: BaseInteractor,
   ...
   */
    private mouseRayCast(ray: RaycastInfo) {
        this.probe.rayCastAll(ray.locus,
            this.endPoint,
   ...
   */
    ...
    // Nothing is targeted if no interactables exist, or we've iterated through all sphere options
        ...
        this.probe.sphereCastAll(this.spherecastRadii[index] * this.targetingVolumeMultiplier,
            castOrigin,
            castEnd,
            (hits: RayCastHit[]) => {
