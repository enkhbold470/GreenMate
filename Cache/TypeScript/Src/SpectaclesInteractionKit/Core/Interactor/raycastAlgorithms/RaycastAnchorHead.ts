...
export default class RaycastAnchorHead extends RaycastBase {
   ...
   */
    private createTransformToWorldFromSituationBasedSpace(up: vec3,
        forward: vec3,
        cervicalLinkPos: vec3): mat4 {
        up = up.normalize();
   ...
   */
    private computeCervicalLink(deviceCenterToWorld: mat4,
        deviceCenterInDeviceSpace: vec3): vec3 {
   ...
   */
    private computeTrasformToWorldFromSituationBasedSpace(deviceCenterToWorld: mat4,
      ...
      this.createTransformToWorldFromSituationBasedSpace(up,
          forward,
          cervicalLinkInWorld);
   ...
   */
    ...
    private applyXRotation(vector: vec3,
        angleRad: number,
        toWorldFromSituationSpace: mat4): vec3 {
    ...
    private applyYRotation(vector: vec3,
        angleRad: number,
        handType: HandType): vec3 {
        const vector4d = new vec4(vector.x, vector.y, vector.z, 1);
    ...
    private determineHandPitchInRadians(wrist: vec3 | null,
        index: vec3 | null,
        middle: vec3 | null): number {
    ...
    private determineGazePitchInRadians(rayAlgorithmData: RayAlgorithmData): number {
        const forward = rayAlgorithmData.deviceCenterToWorld.multiplyDirection(vec3.back());
        const forwardHoriz = Math.sqrt(forward.x * forward.x + forward.z * forward.z);
        return Math.atan2(forward.y, forwardHoriz);
    }

    private determineLowHighRatio(pitchRad: number,
        lowThresholdRad: number,
        highThresholdRad: number): number {
        return MathUtils.clamp(MathUtils.remap(pitchRad, lowThresholdRad, highThresholdRad, 0, 1),
            0,
            1);
    }

    private interpolateFarDirectionStartPoint(handPitch: number,
        lowThreshold: number,
        highThreshold: number,
        pointA: vec3,
        pointB: vec3): vec3 {
        const lowHighRatio = this.determineLowHighRatio(handPitch,
            lowThreshold,
            highThreshold);
        return interpolateVec3(pointA, pointB, lowHighRatio);
    }

    private applyTargetingHeightControl(farDirectionStartPoint: vec3,
        rayAlgorithmData: RayAlgorithmData,
        toWorldFromSituationSpace: mat4): vec3 {
        const hipOffsetFromCervicalLinkInSituationSpace = new vec3(0,
            this.config.hipLevelYOffsetFromCervicalLinkInSituationSpace,
            0);
        const haloOffsetFromCervicalLinkInSituationSpace = new vec3(0,
      ...
      toWorldFromSituationSpace.multiplyDirection(haloOffsetFromCervicalLinkInSituationSpace);
        ...
        if (
            ...
            pitchRad = this.determineHandPitchInRadians(rayAlgorithmData.wrist,
      ...
      (upperThresholdPairLowRad + lowerThresholdPairHighRad) / 2;
        ...
        if (pitchRad > midThresholdRad) {
            farDirectionStartPointUpdated = this.interpolateFarDirectionStartPoint(pitchRad,
                upperThresholdPairLowRad,
                upperThresholdPairHighRad,
    ...
    getRay(): RaycastInfo | null {
        ...
        const cervicalLinkInWorld = this.computeCervicalLink(data.deviceCenterToWorld,
            deviceCenterInDeviceSpace);

        const toWorldFromSituationSpace =
      this.computeTrasformToWorldFromSituationBasedSpace(data.deviceCenterToWorld,
          cervicalLinkInWorld);
      ...
      farDirectionStartPointEnum.CERVICAL_LINK
        ...
        if (this.config.facilitateHighLowTargeting) {
            farDirectionStartPoint = this.applyTargetingHeightControl(farDirectionStartPoint,
                data,
        ...
        let smoothTargetingRay = this.directionOneEuroFilter.filter(targetingRay,
            getTime());
        ...
        if (this.pinchJumpSuppressor) {
            ...
            const rayData = this.pinchJumpSuppressor.applySuppression(smoothTargetingRay,
                filteredLocus);
      ...
      this.irInteractionTransitionStrategy?.computeXRotationInRadians(this.determineGazePitchInRadians(data),
          toWorldFromSituationSpace,
          filteredLocus) ?? 0;

        let normalizedDirection = smoothTargetingRay.normalize();
        normalizedDirection = this.applyYRotation(this.applyXRotation(normalizedDirection,
            this.extraXRotationRad + irInteractionXRotationInRadians,
            toWorldFromSituationSpace).normalize(),
        this.extraYRotationRad,
        this.hand.handType).normalize();
