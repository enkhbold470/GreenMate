...
export class PinchJumpSuppressor {
    ...
    protected stableFingerRootsAverageDistance = new TimedScalarContainer(WindowMode.TIME,
    ...
    protected pinchDistanceTimeSeries = new TimedScalarContainer(WindowMode.FRAME,
    ...
    constructor(protected hand: BaseHand) {
        for (let i = 0; i < this.stableKnuckleKeypoints.length; i++) {
            const series = new TimedVec2Container(WindowMode.TIME,
   ...
   */
    updateState(knuckleSmoothingSpaceFromWorld: mat4): void {
      ...
      this.computeKnucklesDistanceFromSmootingSpaceOrigin(knuckleSmoothingSpaceFromWorld,
          stableKnuckleKeypoints);
        const maxKnuckleMoveIntensity = this.computeMaxKnuckleMoveIntensity(knucklesDistance,
   ...
   */
    private computeKnucklesDistanceFromSmootingSpaceOrigin(knuckleSmoothingSpaceFromWorld: mat4,
        stableKnuckleKeypoints: (Keypoint | null)[]): number | null {
   ...
   */
    private computeMaxKnuckleMoveIntensity(knucklesDistance: number | null,
        knuckleSmoothingSpaceFromWorld: mat4,
        stableKnuckleKeypoints: (Keypoint | null)[]): number {
        ...
        if (knucklesDistance) {
          ...
          depth !== 0
              ? new vec3((pointInSmoothingSpace.x / depth) * knucklesDistance,
                  (pointInSmoothingSpace.y / depth) * knucklesDistance,
                  knucklesDistance)
              : new vec3(pointInSmoothingSpace.x,
          ...
          PinchJumpSuppressorConfigDefault.transformStableKnucklesBackToWorld
              ...
              : stablePointSmoothingSpace;
                ...
                this.stableFingerRootsTimeSeries[i].pushData(getTime(),
        ...
        if (maxKnuckleSpeed !== null) {
            maxKnuckleMoveIntensity = MathUtils.clamp(inverseLerp(this.minKnuckleSpeedCmPerSec,
   ...
   */
    private getPositionIntensity(): number | null {
        ...
        let normalizedRadius = inverseLerp(this.minPinchDistanceCm,
   ...
   */
    private getPinchSpeedDescriptor(): PinchSpeedDescriptor | null {
        ...
        const normalizedSpeed = MathUtils.clamp(Math.abs(this.pinchSpeed) / this.maxPinchSpeedCmPerSec,
            0,
            1);
   ...
   */
    private applyDirectionLowpassFilter(direction: vec3): vec3 {
        ...
        if (this.deltaMode === DeltaMode.MEASURE) {
            this.directionDelta = AxisAngle.getRotationBetween(direction,
                filteredDirection);
   ...
   */
    private applyDirectionDelta(direction: vec3): vec3 {
        return this.deltaMode === DeltaMode.APPLY
            ? AxisAngle.applyRotation(this.directionDelta.multipliedBy(this.deltaMultiplier),
                direction)
            : direction;
    }
   ...
   */
    private updateSuppressionState(): void {
        ...
        const combinedPinchIntensity = MathUtils.clamp(pinchPositionIntensity + pinchSpeedDescriptor.absoluteValue,
            0,
            1);
