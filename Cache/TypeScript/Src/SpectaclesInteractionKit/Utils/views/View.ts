...
export default class View<Config extends ViewConfig = ViewConfig> {
   ...
   */
    ...
    protected addChild<Child extends View<any>>(child: Child,
        configure?: (c: Child) => void,
        preserveWorldTransform: boolean = false): Child {
    ...
    protected newChild<Conf, Child extends View<Conf>>(ctor: {new (conf: Conf): Child},
        conf: Conf,
        customize?: (c: Child) => void): Child {
  ...
  >(fieldName: FieldName, newValue: FieldType) {
        const viewStack: any[] = [...this.childViews];
    ...
    private attachToParent(parent: View<any> | SceneObject,
        preserveWorldTransform: boolean = false) {
    // set the scene object parent
        const parentSceneObject: SceneObject =
    ...
    private cascadeAttachToSceneCallbacks(parentDefaults: Defaults | null = null,
