...
export default class HeadlockRotationCalculator {
    ...
    private calculateRotationOffset(angle: number,
        axisVector: vec3,
        positionVector: vec3): vec3 {
        ...
        if (this.axisEasing !== 1) {
            ...
            newPositionVector = vec3.lerp(positionVector,
                newPositionVector,
   ...
   */
    private calculateAngleDelta(axisVector: vec3,
        positionVector: vec3,
        originVector: vec3,
        forwardVector: vec3,
        upVector?: vec3): number {
        ...
        if (this.axis === RotationAxis.Pitch && upVector !== undefined) {
            const forwardVectorOnXZ = new vec2(forwardVectorOnPlane.x,
                forwardVectorOnPlane.z).normalize();
            const positionVectorOnXZ = new vec2(positionVectorOnPlane.x,
                positionVectorOnPlane.z).normalize();
   ...
   * @returns
   */
    public getOffset(axisVector: vec3,
        positionVector: vec3,
        originVector: vec3,
        forwardVector: vec3,
        upVector?: vec3): vec3 {
        ...
        const angle = this.calculateAngleDelta(axisVector,
            positionVector,
            originVector,
            forwardVector,
            upVector);
        const rotationOffset = this.calculateRotationOffset(angle,
            axisVector,
            positionVector);
