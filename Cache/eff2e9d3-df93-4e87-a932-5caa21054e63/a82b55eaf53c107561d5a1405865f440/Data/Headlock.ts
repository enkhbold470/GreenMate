...
export class Headlock extends BaseScriptComponent {
  ...
  @input
  @hint('If the camera will follow when the user moves their head along XZ-plane')
  private _xzEnabled: boolean = true;
  @input
  @hint('How fast the camera will follow along the XZ-plane, 0.1 for delayed follow, 1 for instant follow.')
  ...
  @input
  @hint('How fast the camera will follow along the Y-axis, 0.1 for delayed follow, 1 for instant follow.')
  private _yEasing: number = 1;
  @input
  @hint('The magnitude of change needed to activate a translation for the target to follow the camera.')
  @ui.group_end
  private _translationBuffer: number = 0;
  @ui.group_start('Head Rotation')
  @input
  @hint('If the camera will NOT follow when the user moves their head along the pitch-axis (looking up/down)')
  private _lockedPitch: boolean = true;
  @input
  @hint('How fast the camera will follow along the pitch-axis, 0.1 for delayed follow, 1 for instant follow.')
  private _pitchEasing: number = 1;
  @input
  @hint('How many degrees of offset from the center point should the target sit')
  ...
  @input
  @hint('If the camera will follow when the user moves their head along the yaw-axis (looking left/right)')
  private _lockedYaw: boolean = true;
  @input
  @hint('How fast the camera will follow along the yaw-axis, 0.1 for delayed follow, 1 for instant follow.')
