...
export const easingFunctions = {
    ...
    'ease-in-out-back-cubic': createCubicBezierEasingFunction(0.3,
        -0.25,
        0.6,
        1.42),
} as const;
...
export default function animate(config: AnimateConfig): CancelFunction {
    let running = true;
    let elapsedTime = 0;
    const easingFunction = easingFunctions[config.easing ?? 'linear'];
...
export function mix(a: any, b: any, t: number) {
    if (typeof a === 'number') {
        ...
        return new vec3(mixNumbers(a.x, b.x, t),
            mixNumbers(a.y, b.y, t),
            mixNumbers(a.z, b.z, t));
    } else if (b instanceof vec4) {
        return new vec4(mixNumbers(a.x, b.x, t),
            mixNumbers(a.y, b.y, t),
            mixNumbers(a.z, b.z, t),
 ...
 */
function createCubicBezierEasingFunction(x1: number,
    y1: number,
    x2: number,
    y2: number): (t: number) => number {
