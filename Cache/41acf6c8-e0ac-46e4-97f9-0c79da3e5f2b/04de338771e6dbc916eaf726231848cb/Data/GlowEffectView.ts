...
export class GlowEffectView {
    ...
    private hand: TrackedHand = this.handProvider.getHand(this.config.handType as HandType);
    ...
    private handToTap: TrackedHand = this.handProvider.getHand(this.config.handType === 'left' ? 'right' : 'left');
   ...
   */
    private indexQuadSceneObject: SceneObject = this.setupTipQuadSceneObject(this.indexTipSceneObject,
        'indexTipQuadSceneObject');
    private indexQuadTransform: Transform =
        this.indexQuadSceneObject.getTransform();
    private thumbQuadSceneObject: SceneObject = this.setupTipQuadSceneObject(this.thumbTipSceneObject,
   ...
   */
    ...
    // Connect to ViewModel events. This is how the glow effect will be driven
        this.glowEffectViewModel.animateIndexGlowBase.add((animateGlowBaseUp: boolean) => {
            this.animateIndexGlowBase(animateGlowBaseUp);
        });
        this.glowEffectViewModel.animateIndexGlowBonus.add((animateGlowBonusUp: boolean) => {
            this.animateIndexGlowBonus(animateGlowBonusUp);
        });
        this.glowEffectViewModel.animateThumbGlowBase.add((animateGlowBaseUp: boolean) => {
            this.animateThumbGlowBase(animateGlowBaseUp);
        });
        this.glowEffectViewModel.animateThumbGlowBonus.add((animateGlowBonusUp: boolean) => {
        ...
        this.updateDispatcher
            ...
            .bind(() => {
                ...
                if (this.hand.getPinchStrength() === 0) {
                    ...
                    this.indexQuadTransform.setWorldRotation(quat.lookAt(cameraPosition.sub(this.indexQuadTransform.getWorldPosition()),
                        VEC3_UP));
                    this.thumbQuadTransform.setWorldRotation(quat.lookAt(cameraPosition.sub(this.thumbQuadTransform.getWorldPosition()),
                        VEC3_UP));
                    if (!this.indexQuadSceneObject.enabled) {
   ...
   */
    ...
    private setupTipQuadSceneObject(parentSceneObject: SceneObject,
        sceneObjectName: string): SceneObject {
    ...
    private applyMaterialUpdates(combinedIndexGlowFactor: number,
        combinedThumbGlowFactor: number): void {
    ...
    private updateMaterialTintColor(indexGlowBonus: number,
        thumbGlowBonus: number) {
        const indexTintColor = vec4.lerp(this.config.idleColor,
            this.config.pinchDownColor,
            indexGlowBonus);

        const thumbTintColor = vec4.lerp(this.config.idleColor,
            this.config.pinchDownColor,
            thumbGlowBonus);
