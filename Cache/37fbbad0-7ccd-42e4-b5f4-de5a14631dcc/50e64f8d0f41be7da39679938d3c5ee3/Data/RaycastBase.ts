...
export default abstract class RaycastBase {
   ...
   */
    calculateCastAnchor(thumbKnuckle: vec3, middleKnuckle: vec3): vec3 {
        const middleFingerThumbOffset: vec3 = middleKnuckle.sub(thumbKnuckle);
        const rootMiddleFinger: vec3 = this.knuckleOneEuroFilter
   ...
   */
    protected calculateInteractionLocus(thumbKnuckle: vec3,
        indexKnuckle: vec3): vec3 {
        return indexKnuckle
   ...
   */
    protected getCorrectedHandLandmarkPosition(cameraToWorld: mat4,
        avgZ: number,
        filteredAvgZ: number,
        worldPos: vec3 | null): vec3 | null {
        if (worldPos === null) {
            return null;
        }
        ...
        const newCameraPos = new vec3((cameraPos.x / cameraPos.z) * newZ,
   ...
   */
    ...
    // Get joints from hands api
        ...
        const getCorrectedPosition = (position: vec3 | null) => {
            return this.getCorrectedHandLandmarkPosition(cameraToWorld,
