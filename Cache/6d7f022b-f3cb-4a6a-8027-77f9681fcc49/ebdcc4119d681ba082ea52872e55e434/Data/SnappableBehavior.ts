...
export class SnappableBehavior {
    ...
    constructor(private options: SnappableOptions) {
        ...
        const boundingBoxCollider: ColliderComponent =
      this.boundingBox.createComponent('ColliderComponent');
        ...
        if (!this.useConstantBoundingBoxPadding) {
          ...
          this.frame.border);
            ...
            this.boundingBoxTransform.setWorldScale(new vec3(frameWorldScale.x,
                frameWorldScale.y,
    ...
    // the padding of the constant size added to the bounding box
        const frameWorldScale = new vec2(this.frame.innerSize.x + this.frame.constantPadding.x + this.frame.border,
            this.frame.innerSize.y + this.frame.constantPadding.y + this.frame.border);
        const boxPadding = 10;
        const boxDepth = Math.max(Math.min((Math.max(frameWorldScale.x, frameWorldScale.y), 40)),
            1);
        this.boundingBoxTransform.setWorldScale(new vec3(frameWorldScale.x + boxPadding,
    ...
    // if not set
        ...
        if (this.aspectRatio > 1) {
            ...
            this.ghostImageComponent.mainMaterial.mainPass.boxSize = new vec2(0.5,
                0.5 / this.aspectRatio);
        } else {
            scale.x /= this.aspectRatio;
            this.ghostImageComponent.mainMaterial.mainPass.boxSize = new vec2(0.5 * this.aspectRatio,
                0.5);
        }
    ...
    private queryWorld = () => {
        ...
        if (this.hitTestSessionStarted) {
            const hitTest = this.hitTestSession.hitTest(indexTipPos,
                endPos,
                (hitResult) => {
                    if (hitResult === null) {
              ...
              containerPosition.distanceSquared(hitPosition);
                        ...
                        if (this.queryBuffer.length === this.queryBufferMaxLength) {
                            ...
                            if (avgNormal.y > 0.95) {
                                ...
                                toRotation = quat.lookAt(avgNormal,
    ...
    private calculateItemSnapping = () => {
        ...
        this.gutterSize / snapToScalar.x;
            ...
            if (Math.abs(this.contentPosition.y) < this.stickyZoneSize) {
                ...
                Math.abs(this.contentPosition.y +
        ...
        this.gutterSize / snapToScalar.y;
            ...
            if (Math.abs(this.contentPosition.x) < this.stickyZoneSize) {
                ...
                Math.abs(this.contentPosition.x +
            halfThisImageScale.x -
