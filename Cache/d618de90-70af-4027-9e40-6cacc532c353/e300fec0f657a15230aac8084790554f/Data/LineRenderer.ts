...
export default class LineRenderer extends View<LineViewConfig> {
   ...
   */
    updateVertexPosition(index: number, newPos: vec3) {
        ...
        this.meshVertexData[
            ...
            const prevSegment = utils.getSegmentNormalized(this._points[index - 1],
                this._points[index]);
            const nextSegment = utils.getSegmentNormalized(this._points[index],
        ...
        this.meshBuilder.setVertexInterleaved(index * VERTICES_PER_POINT,
            leftVertexData);
        this.meshBuilder.setVertexInterleaved(index * VERTICES_PER_POINT + 1,
            rightVertexData);
   ...
   */
    private buildGeometry() {
        ...
        const vertexData: number[] = Array(vertexArraySize);
