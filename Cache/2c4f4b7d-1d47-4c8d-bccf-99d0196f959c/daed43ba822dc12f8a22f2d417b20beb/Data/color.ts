...
export function parseColor(str: string, alpha: number = 1.0): vec4 {
    ...
    return new vec4(parseInt(str.substr(0, 2), 16) / 255,
...
export function getMostCommonHue(texture: Texture): number {
    ...
    const colors = new Array<number>(bucketCount);
    ...
    const samples = new SeededRandomNumberGenerator(pixels).getRandomNumberArrayInRangeNoDuplicates(0, pixels - 1, sampleCount);
