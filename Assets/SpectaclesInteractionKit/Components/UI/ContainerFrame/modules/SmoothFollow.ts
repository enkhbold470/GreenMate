...
export class SmoothFollow {
    ...
    // this component when grabbed, and reenables it when let go.
        ...
        this.target.z = Math.max(this.target.z,
            (1.1 * this.visibleWidth) /
        2 /
        Math.tan((this.fieldOfView / 2) * DegToRad)); // handle very wide panels
        ...
        const halfFov = Math.atan((Math.tan((this.fieldOfView / 2) * DegToRad) * dist -
        this.visibleWidth / 2) /
        this.target.z);
    ...
    onUpdate() {
        if (!this.dragging) {
            ...
            ;[pos.x, this.velocity.x] = smoothDamp(pos.x,
                this.target.x,
                this.velocity.x,
                this.translationTime,
                getDeltaTime())
            ;[pos.y, this.velocity.y] = smoothDamp(pos.y,
                this.target.y,
                this.velocity.y,
                this.translationTime,
                getDeltaTime())
            ;[pos.z, this.velocity.z] = smoothDamp(pos.z,
                this.target.z,
                this.velocity.z,
                this.translationTime,
                getDeltaTime());
            this.worldPos = this.bodyToWorld(this.cylindricalToCartesian(pos))
            ;[this.heading, this.omega] = smoothDampAngle(this.heading,
                this.cameraHeading,
                this.omega,
                this.rotationTime,
                getDeltaTime());
    ...
    private cartesianToCylindrical(v: vec3): vec3 {
        return new vec3(Math.atan2(-v.x, -v.z),
            v.y,
