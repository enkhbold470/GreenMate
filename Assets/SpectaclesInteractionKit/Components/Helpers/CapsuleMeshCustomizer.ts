...
export class CapsuleMeshCustomizer extends BaseScriptComponent {
  ...
  @input('int')
  @hint('The number of circles in the end cap of the capsule of the mesh. Increase for a higher poly-count mesh.')
  ...
  private linkEndIndices(endIndex: number,
      circleIndex: number,
      isRight: boolean) {
      let indices: number[] = [];
      const numPoints = this.radianStepCount * 2;
  ...
  private buildCapsule() {
      ...
      const leftEndCap = this.buildEndCap(-this.capsuleLength / 2,
          this.radius,
          false);
      const cylinder = this.buildCylinder(this.capsuleLength, this.radius);
      const rightEndCap = this.buildEndCap(this.capsuleLength / 2,
          this.radius,
          true);
      ...
      this.builder.appendIndices(this.linkEndIndices(this.builder.getVerticesCount() - 1,
