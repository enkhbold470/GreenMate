...
export default class RadialOcclusionView {
   ...
   */
    ...
    private gradientScaleFactorMovingAverage = new MovingAverageFilter<number>(GRADIENT_SCALE_FACTOR_MOVING_AVERAGE_SIZE,
        () => 0,
        (samples: number[]) => {
   ...
   */
    ...
    private setupGradientQuadSceneObject(sceneObjectName: string): SceneObject {
        ...
        quadSceneObject
            ...
            .setLocalPosition(this.config.handType === 'left'
                ...
                : new vec3(-1 * GRADIENT_QUAD_OFFSET_LEFT.x,
      ...
      this.config.radialGradientOcclusionMaterial.clone();
        ...
        const billboardComponent = quadSceneObject.createComponent(Billboard.getTypeName());
    ...
    private updateGradientScaleFactor() {
      ...
      3;
        ...
        const clampedAverageDistance = MathUtils.clamp(averageDistance,
            MIN_MEAN_AVG_FINGER_DISTANCE,
            MAX_MEAN_AVG_FINGER_DISTANCE);

        const scaledAverageDistance = MathUtils.remap(clampedAverageDistance,
            MIN_MEAN_AVG_FINGER_DISTANCE,
            MAX_MEAN_AVG_FINGER_DISTANCE,
      ...
      this.gradientScaleFactorMovingAverage.filter(scaledAverageDistance,
          Date.now());
    }
    ...
    private updateVisuals() {
      ...
      this.gradientQuadSceneObject.getComponent('RenderMeshVisual');
        ...
        gradientQuadRenderMeshComponent.mainMaterial.mainPass.baseColor = withAlpha(gradientQuadRenderMeshComponent.mainMaterial.mainPass.baseColor,
            this.gradientOcclusionOpacity);

        const gradientQuadLocalScale = GRADIENT_QUAD_LOCAL_SCALE.uniformScale(MathUtils.remap(this.currentAverageScaleFactor,
            GRADIENT_SCALE_FACTOR_MIN,
            GRADIENT_SCALE_FACTOR_MAX,
