 ...
 */
export const debounce = (debouncedFunction: (...args: AnyArg) => void,
    timeoutInMsecs: number) => {
    let timeoutId: ReturnType<typeof setTimeout>;
    return (...args: AnyArg): void => {
        clearTimeout(timeoutId);
        timeoutId = setTimeout(() => debouncedFunction.apply(this, args),
            timeoutInMsecs);
    };
};
 ...
 */
export const throttle = (throttledFunction: (...args: AnyArg) => void,
    delayMsecs: number) => {
    let timeoutId: ReturnType<typeof setTimeout>;
    let previousTime = 0;
    ...
    return (...args: AnyArg): void => {
        ...
        if (currentTime - previousTime < delayMsecs) {
            clearTimeout(timeoutId);
            timeoutId = setTimeout(() => throttledFunction.apply(this, args),
                delayMsecs);
        } else {
