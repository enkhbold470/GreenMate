...
implements IrInteractionTransition {
    ...
    /** @inheritdoc */
    computeXRotationInDegrees(gazePitchInDegrees: number,
        toWorldFromSituationSpace: mat4,
        handPoint: vec3): number {
    ...
    /** @inheritdoc */
    computeXRotationInRadians(gazePitchInRadians: number,
        toWorldFromSituationSpace: mat4,
        handPoint: vec3): number {
        return (
            MathUtils.DegToRad *
      this.computeXRotationInDegrees(MathUtils.RadToDeg * gazePitchInRadians,
   ...
   */
    private computeMultiplier(handPoint: vec3,
        gazePitchInDegrees: number): number {
   ...
   * @param gazePitchInDegrees - gaze pitch given in degrees
   */
    private switchTransitionStrategyInDefaultMode(handPoint: vec3,
        gazePitchInDegrees: number): void {
        if (
      ...
      gazePitchInDegrees > -this.config.gazePitchDegreesToIr
            ...
            this.irModeSaturationProgress = MathUtils.clamp(this.irModeSaturationProgress,
                0,
                1);
            this.debounceProgress -= getDeltaTime() / this.debounceToIrSeconds;
            this.debounceProgress = MathUtils.clamp(this.debounceProgress, 0, 1);
   ...
   * @param gazePitchInDegrees - gaze pitch given in degrees
   */
    private switchTransitionStrategyInIrMode(handPoint: vec3,
        gazePitchInDegrees: number): void {
        if (
